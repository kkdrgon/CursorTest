...
### 10. 移动端适配策略
- 采用 `flex` + 媒体查询在 <=900px 时切换为纵向布局，Canvas 100% 宽度、自动高度。
- 控件区域支持折叠（按钮只在窄屏显示），并隐藏信息面板/日志，保留核心画面。
- Canvas 启用 `touch-action: none` 并监听 `pointerdown`，第一触点即更新发射点，减少点击延迟。
- 提升控件可触性：按钮最小宽度、字段垂直排列、信息网格单列，满足指尖操作空间。

### 11. 法师结印系统
- 在控制面板新增 `sealCanvas`，提示玩家绘制「▽」形手势，采用模板采样 + 轨迹归一化方式计算拟合度。
- 结印成功后生成 buff（限定移动距离 + 存续时间）；火球术在 buff 生效且角色未移动时可连发，威力按拟合度 0~80% 增益。
- 火球术、陨石术在 `params` 中读取 `sealPower`、`sealRapidFire`，自适应调整伤害、射程或引导时间，并输出提示文案。
- 提供清除按钮与状态提示，便于在触屏上重新绘制；移动发射点会自动使结印失效，保证「不动即可连发」的设计语义。
